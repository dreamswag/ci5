...
# Version: 2.0-PHOENIX (Merged - Procd + Full Features)
#
...
show_plan() {
    ...
    local svc_count=$(ls $CI5_INIT 2>/dev/null | wc -l)
    [ "$svc_count" -gt 0 ] && printf "  ${R}✗${N} %d Procd service(s)\n" "$svc_count"
...
remove_infrastructure() {
    ...
    # Stop and remove Procd services
    for service in $CI5_INIT; do
        [ -f "$service" ] || continue
        ...
        if $DRY_RUN; then
            ...
            "$service" disable 2>/dev/null || true
            rm -f "$service"
        fi
    done
...
secure_delete() {
    ...
    if [ -f "$target" ]; then
        ...
        if [ "$storage" = "hdd" ] && command -v shred >/dev/null 2>&1; then
...
wipe_sensitive() {
    ...
    # Stop sensitive services first (Procd)
    /etc/init.d/wireguard stop 2>/dev/null || true
