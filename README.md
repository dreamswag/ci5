...
<details> <summary>📡 <b>Expand generic AP configuration</b></summary>

Check generic_ap_reference.txt (generated by the Wizard). Your goal is to map SSIDs to VLAN IDs.

1. The Controller Logic (UniFi / Omada)
These systems use a "Controller" to push settings to APs.

Create Networks (VLAN-Only / Third-Party Gateway):

Name: Trusted | VLAN ID: 10

Name: IoT | VLAN ID: 30

Name: Guest | VLAN ID: 40

Note: Do NOT enable "DHCP Server" on the controller. The Pi 5 handles all DHCP.

Create Wi-Fi Networks (SSIDs):

SSID: Ci5_Trusted -> Network: Trusted (VLAN 10)

SSID: Ci5_IoT -> Network: IoT (VLAN 30) -> Client Isolation: ON

SSID: Ci5_Guest -> Network: Guest (VLAN 40) -> Client Isolation: ON
...
2. The "Router as AP" Logic (Asus / Netgear Stock)
...
This automatically disables the Firewall, NAT, and DHCP Server.

VLAN Support:

Warning: Most stock consumer routers CANNOT assign VLANs to SSIDs.

If your router does not support "VLAN to SSID Mapping" (often called "IPTV/VLAN" or "Multi-SSID"), everything will connect to the Trusted Network (VLAN 10) or Management Network.

Fix: Flash OpenWrt on it (See Option A) or buy a VLAN-capable AP (UniFi U6, Omada EAP).

3. Physical Wiring (Critical)
