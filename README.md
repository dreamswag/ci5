...
|  | **Full:** USB 3.0 Flash/SSD highly recommended. |  |
...
## 💾 Phase 3: **<u>Installation (One-Click)</u>** 🛣️

### 🏁 Step 1: Flash The Brain

We utilize a pre-baked "Golden Master" image. This contains all drivers, Python dependencies, and the Ci5 scripts pre-loaded.

1. **Download:** Go to **[Releases](https://github.com/dreamswag/ci5/releases)** and grab the latest `Ci5-Pi5-v7.4-RC1.img.gz`.
...
1. **Connect:** Ensure your PC is connected to the Pi 5 LAN port (via the AP or direct).
...
3. **Launch:**
   ...
   cd ci5 && sh setup.sh
   ```
   *Answer the questions (ISP Type, WiFi Password, etc).*
...
> **Status:** You now have a working, tuned router. 🛜
...
## 🛜 Phase 4: **<u>Access Point Setup</u>** 🪄
...
*(The R7800 will download its config from the Pi 5, apply VLANs, and reboot into a Dumb AP.)*
...
<details>
<summary>📡 <b>Click to expand generic AP configuration</b></summary>

Check `generic_ap_reference.txt` (generated by the Wizard), and configure AP Controller:

* **VLANs for LAN interface:**
  
  * **Trusted** → VLAN **10**
    * LAN Port 1 (tagged), LAN Port 2 (untagged)
  * **IoT** → VLAN **30** (Client Isolation: ON)
    * LAN Port 1 (tagged), LAN Port 3 (untagged)
  * **Guest** → VLAN **40** (Client Isolation: ON)
    * LAN Port 1 (tagged)
  * **Pi 5 (Trunk)** → VLAN **1**
    * LAN Port 1 (untagged), LAN Port 4 (untagged)
...
* **Interfaces**: 
  ...
  * **WAN** → **Disabled**
  * **LAN** → **Enabled** (Static Address)
    * ⚠️ **CHANGE THIS LAST - AS APPLYING WILL DISCONNECT SSH/WEB UI ACCESS** ⚠️
      * (*due to device IP being changed to 192.168.99.2*)

    * **IPv4 Address**: 192.168.99.2/24
    * **IPv4 Gateway**: 192.168.99.1
    * **Custom DNS Server**: 192.168.99.1
    * **Firewall Zone**: LAN
    * **DHCP Server**: Enabled

  * **Trusted** → **Enabled** (Static Address)
    * **Device**: br-lan.10
    * **IPv4 Address**: 10.10.10.2/24
    * **IPv4 Gateway**: 10.10.10.1
    * **Custom DNS Server**: 10.10.10.1
    * **Firewall Zone**: LAN
    * **DHCP Server**: Disabled
  * **IOT** → **Enabled** (Unmanaged)
    * **Device**: br-lan.30
    * **DHCP Server**: Disabled

  * **Guest** → **Enabled** (Unmanaged)
    * **Device**: br-lan.40
    * **DHCP Server:** Disabled
...
* **Firewall**:
  ...
  * **Zones:** 
    ...
    * **WAN**:
      * **Input**: reject; **Output**: reject; **Forward**: reject
      * **Covered Networks**: WAN
      * **Allow forward from destination zones**: wan
...
* **Wireless**:
  * **Create different SSID profiles on 5.2Ghz / 2.4Ghz bands**
  * You can assign multiple SSIDs to the same band - for example:
    * **5.2Ghz** band for '**trusted**' interface
      * (**Hide ESSID**: enabled)
      * (**Client Isolation**: enabled)
        * (disable if you need cross-device discoverability - e.g. **LANDrop**)
          * (*but ensure that you can absolutely trust all devices on this network*)
      * (**Encryption**: WPA3-SAE)
    * **2.4Ghz** band for '**iot**' interface
      * (**Hide ESSID**: disabled)
      * (**Client Isolation**: enabled)
      * (**Encryption**: WPA2-PSK/WPA3-SAE Mixed)
    * **5.2Ghz** band for '**guest**' interface 
      * (**Hide ESSID**: disabled)
      * (**Client Isolation**: enabled)
      * (**Encryption**: WPA2-PSK/WPA3-SAE Mixed)
...
**REWIRE:** Once finished, move the **Pi 5** (eth0 internal) cable to **AP LAN 1** as per the Wiring Guide.
...
### For the "I build my own binaries" crowd.
...
Or build it yourself at **[firmware-selector.openwrt.org](https://firmware-selector.openwrt.org/)** → **Raspberry Pi 5** → **24.10.4**.

**CRITICAL: Use the EXT4 image. SquashFS is read-only and will brick this workflow.**

<details>
<summary>📦 <b>Click to expand Package List (for custom builds)</b></summary>
...
<details>
<summary><b>"Why is my PC on R7800 Port 2 not working?"</b></summary>
...
<details>
<summary><b>"My Bufferbloat is 'B' or 'C', not 'A+'?"</b></summary>
...
<details>
<summary><b>"How do I update the stack?"</b></summary>
